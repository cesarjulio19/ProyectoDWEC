...
class Menu extends Phaser.Scene {
    ...
    create(){
        ...
        //A=B+C
        this.allTextures = ["mario", "peach", "luigi","bowser"];
        this.activeTextures = [];
        ...
        this.socket.on('currentPlayersMenu', function (players) {
...
updateActivePlayers(players,self){
    ...
    for (let i = 0; i < self.availableTextures.length; i++) {
        if((i==0)||(i==1)){
          ...
          .sprite(i * 150 + 150, 465, self.availableTextures[i])
          .setOrigin(0, 0);
          
        }else{
            self.availableSprites[i] = this.add
          .sprite(i * 200 + 200, 465, self.availableTextures[i])
          .setOrigin(0, 0);
